...
// Hook : 함수형 컴포넌트에서 React의 상태, 설명주기, 로직을 사용할 수 있는 API(함수)
// - React Hook : 리액트에서 미리 만들어진 함수(Hook)
// - ex) useState, useQuery, useMutation, ...
// - 커스텀 Hook : 개발자가 직접 만든 함수(Hook) tanstack query도 훅임 쿼리로 use사용해서 빼둠
...
export const useNoticeListHook = () => {
    ...
    // 공지사항 리스트 조회
    const { data, isLoading, isError, error } = useNoticeListQuery(curruntPage);
    ...
    const goToPrevPage = () => {
        if(curruntPage > 1) {
            handlePageChange(curruntPage -1);
        }
    }
    ...
    const pageNumbers = useMemo(() => {
        const pages = [];
        const pageGroup = Math.ceil(curruntPage / 5);
